<template>
	<view class="second_view_questions">
		<uni-easyinput :maxlength="-1" :disabled="disabled" @input="changeInput" v-model="aaList" type="textarea"
			autoHeight placeholder="请输入内容">
		</uni-easyinput>
...
<script>
	export default {
		...
		methods: {
			addList() {
				this.aaList += '\n问:\n答:'
				this.$emit("change", this.aaList)
			},

			changeInput(e) {
				this.aaList = e
				// console.log("this.aaList", e)
				this.$emit("change", this.aaList)
			}
		},
		props: {
